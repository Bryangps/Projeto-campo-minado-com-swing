...
public class Tabuleiro  implements CampoObervador{
    ...
    public void notificarObervador(boolean resultado) {
        observadores.forEach(o -> o.accept(new ResultadoEvento(resultado)));
    }

    public void abrir(int linha, int coluna) {
        campos.stream()
                .filter(c -> c.getLinha() == linha && c.getColuna() == coluna).findFirst().ifPresent(Campo::abrir);
    ...
    public void eventoOcorreu(Campo campo, CampoEvento evento) {
        if (evento == CampoEvento.EXPLODIR) {
            mostrarMinas();
            notificarObervador(false);
        } else if (objetivoAlcancado()) {
            notificarObervador(true);
        }
    }
